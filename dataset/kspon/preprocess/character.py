...
def generate_character_labels(transcripts, labels_dest):
    ...
    label['id'] = label['id'][:1391]
    label['char'] = label['char'][:1391]
    label['freq'] = label['freq'][:1391]

    label_df = pd.DataFrame(label)
    label_df.to_csv(os.path.join(labels_dest, "SKL_character_vocabs.csv"), encoding="utf-8", index=False)


def generate_character_script(audio_paths, transcripts, labels_dest):
    print('create_script started..')
    char2id, id2char = load_label(os.path.join(labels_dest, "SKL_character_vocabs.csv"))
